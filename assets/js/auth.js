...
// Hamburger toggle + active-link highlight + account-menu dropdown behavior
// — all wired regardless of auth state.
...
export function wireNav(user, profile) {
  ...
  const guestIcon = document.querySelector("[data-nav-guest-icon]");
  ...
  const loginLink = document.querySelector("[data-account-login-link]");

  if (user && profile) {
    if (avatar) { avatar.src = profile.photoURL || user.photoURL || ""; avatar.style.display = ""; }
    if (guestIcon) guestIcon.style.display = "none";
    if (name) { name.textContent = profile.name || user.displayName || ""; name.style.display = ""; }
    ...
    if (guestIcon) guestIcon.style.display = "";
