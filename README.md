...
- The leaderboard shows **every** student who has attempted the selected
  exam (not just a top-N slice), sorted by score in the browser after a
  single equality-filtered read.
...
"আমার ফলাফল" page. Every attempt after that is treated as free practice:
the score/review still shows normally on the result page, it's just never
saved, so students can re-practice an exam as many times as they like
without messing up the leaderboard or their official record. The result
page tells them plainly which case they're in.
